...
# 繁簡摺疊。只在比對時折，**不影響任何要顯示給使用者的字**——
# `normalize_text` 的產物只拿去查表，呈現一律用知識庫的正式名或標示原文。
try:
    from opencc import OpenCC
    _FOLD = OpenCC("t2s").convert
except Exception:      # 套件缺席或字典載入失敗都退回不摺疊
    _FOLD = None
    print("[WARNING] 未載入 opencc，添加物比對不做繁簡摺疊——"
          "辨識層吐出簡體字時會配不到（例：柠檬酸 vs 檸檬酸）")
...
def normalize_text(text: str) -> str:
    ...
    # ⚠ **繁簡摺疊。** 辨識層會吐出簡體字（`trad-decode-finding` 記錄過繁簡混雜
    #    33 案），而知識庫是繁體，於是「柠檬酸」配不到「檸檬酸」。
    #    實測（2026-09-12）：簡體輸入 ['L-麸酸钠','柠檬酸','碳酸氢钠'] 只配到 1/3，
    #    摺疊後 3/3。**兩邊都摺**（查詢與知識庫走同一個函式），所以繁體輸入不受影響。
    #    評估台的 `PPOCR_TEST/sim_match.py` 2026-09-10 就加了，線上漏掉——
    #    這是同一份知識放兩個地方的第三次代價（見 文件與程式落差對照表 六之二）。
    #    ⚠ opencc 缺席時退回不摺疊，行為與先前相同；不讓比對層因為一個
    #      選用套件而整個掛掉。
