...
# 逾時更難診斷。Node 層 105 秒放棄，連線 5 + 讀取 90 = 95 秒，
# 留 10 秒給本機降階跑完並回傳。
#
# 2026-09-13：45 → 90。A5 當初從 90 降到 45，理由是「Cloudflare 會即時回
# 502/530，不必再等」，那在 Gemini 時代成立（Cloud 端 13～16 秒）。
# 改用 vlcrop 之後 Cloud 端要 **10.3 秒／張**（實測穩態五次 ±0.1），
# 三張約 31 秒——45 秒雖然大多數時候夠，但沒有留給熱節流與多張的餘裕。
# ⚠ 同一天稍早量到 55 秒／張，那是 VRAM 擠爆造成的（llama.cpp 把層搬回 CPU），
#   已用參數解掉；細節見 CLAUDE.md 的延遲表。**引用秒數前先看那張表。**
